            ...
            <form name='invoice' id='invoice' action='Invoice.php' method='post'>
				...
				<div class="login-group">
					...
					<div class="form-group">
						...
						<input type="int" class="form-control" id="month" name="month" placeholder="Month (Enter as digits 1-12)">
					</div>
				</div>
			<div class="etc-login-form">
				<input type="submit" id='invoice' name='invoice' value='Submit' class="btn btn-default"/>
			</div>
                </div>
                    </form>
                </div>
...
if(isset($_SESSION['id'])){
   // include database connection
	...
	// SELECT query
        $query = "SELECT date, make_model, length_of_reservation, (length_of_reservation*rental_fee) AS Rental_Fee FROM reservation NATURAL JOIN car WHERE memberID = '".$_POST["memberID"]."' AND month(date) ='".$_POST["month"]."'";
        ...
        // Execute the query
		$stmt->execute();
        $stmt->bind_result($date,$makemodel,$days,$rental_fee);
        echo "<h3>".$_POST["memberID"]."'s monthly invoice for ".$_POST["month"]." is: </h3>";
		echo "<h3>Monthly membership fee: $5</h3>";
...
if(isset($_SESSION['id'])){
   // include database connection
	...
	if (isset($_POST['invoice'])) {
	// SELECT query
        $query = "SELECT Month(date) AS Month, sum(length_of_reservation*rental_fee)+5 AS Total FROM reservation NATURAL JOIN car WHERE memberID = '".$_POST["memberID"]."' AND month(date) ='".$_POST["month"]."'";
        ...
        while ($stmt -> fetch()){
            echo "<h2>Total Payment: $$total</h2>";
        }
